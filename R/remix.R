...
#' partial posterior.
#' @param f the log likelihood function. See details.
...
n_samples_used_each <- sapply(theta, nrow)
if (any(n_samples_used_each < 1)) stop("Insufficient useable samples!")
n_samples_used_total <- sum(n_samples_used_each)
# Dimension of the model.
d <- ncol(theta[[1]])
# Record which partial posterior each sample came from.
pp_inds <- rep(1:n_shards, n_samples_used_each)
...
params <- list(
  ...
  n_samples = n_samples_used_total,
  n_samples_used_each = n_samples_used_each,
  pp_inds = pp_inds,
  f = f,
  d = d
)
...
w.denominator <- matrix(ll.array[c(outer(1:n_samples_used_total, (0:(dim(ll.array)[2] - 1)) * params$n_samples, FUN = "+")) + (pp_inds - 1) * params$n_samples * dim(ll.array)[2]], n_samples_used_total, dim(ll.array)[2])
...
if (type == 2) {
  ...
  # with multiplication by n samples in the mixtu
}
...
#' Compute log likelihood using a single shard of data
#'
#' Worker function for computing the log likelihoods when data are distributed.
#'
